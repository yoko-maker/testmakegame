...
/* 意識転写ラボ ── アンバー（琥珀）の無菌・施設端末トーン。
   アーケードのネオン(マゼンタ/シアン)とは別系統。グローは抑え、
   グリッド主体の「臨床的な研究端末」の質感にする。 */
.stApp {
    background:
        radial-gradient(circle at 50% -8%, rgba(255,176,64,0.07), transparent 52%),
        #0c0a06;
    color: #ddd0b4;
    font-family: 'Share Tech Mono', monospace;
}

/* グリッド線オーバーレイ（施設の方眼。操作を妨げない） */
.stApp::before {
    content: "";
    position: fixed;
    inset: 0;
    ...
    z-index: 0;
    background-image:
        linear-gradient(rgba(255,176,64,0.06) 1px, transparent 1px),
        linear-gradient(90deg, rgba(255,176,64,0.06) 1px, transparent 1px);
...
h1, h2, h3 {
    ...
    text-shadow: 0 0 2px rgba(255,176,64,0.45);
}
h1 {
    border-bottom: 1px solid rgba(255,176,64,0.3);
    padding-bottom: 0.3rem;
}

p, li, label, .stMarkdown, .stCaption { color: #cdbf9d !important; }

/* 端末コンソール風ボタン（フラットで硬い質感） */
.stButton > button, .stDownloadButton > button {
    background: rgba(34,24,10,0.8);
    color: #ffc77a;
    border: 1px solid #b3781f;
    border-radius: 2px;
    font-family: 'Share Tech Mono', monospace;
    letter-spacing: 1px;
    text-transform: uppercase;
...
.stButton > button:hover, .stDownloadButton > button:hover {
    ...
    color: #160f04;
    border-color: #ffb454;
    box-shadow: 0 0 10px rgba(255,176,64,0.5);
}
...
.stTextInput input, .stNumberInput input, .stTextArea textarea {
    ...
    border: 1px solid #6e4d18 !important;
...
.stTextInput input:focus, .stNumberInput input:focus, .stTextArea textarea:focus {
    ...
    box-shadow: 0 0 8px rgba(255,176,64,0.4) !important;
}

/* 進捗バー・区切り線・展開パネルもテーマに合わせる */
.stProgress > div > div > div { background: #ffb454 !important; }
hr { border-color: rgba(179,120,31,0.3) !important; }
[data-testid="stExpander"] {
    border: 1px solid rgba(179,120,31,0.32) !important;
    background: rgba(30,22,8,0.55) !important;
    border-radius: 3px;
}

/* サイドバー（施設ステータス端末風） */
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0e0b05, #1a1206) !important;
    border-right: 1px solid rgba(179,120,31,0.32);
}

/* 入力プレースホルダ */
::placeholder { color: rgba(255,176,64,0.38) !important; }
</style>
"""
...
def setup_logiclock(answer):
    """2桁(00〜99)の暗証番号を、複数の論理条件から“一意に”特定できる錠を生成する。

    プレイヤーが最終的にコンソールへ入力するのは、その2桁の**一の位**であり、
    それが認証コードの該当桁(answer)に一致するように暗証番号を選ぶ。
    （認証フロー・echo_code・echo_digits は一切変えない。）

    難易度: 4〜5個の論理条件を提示し、それらを“同時に”満たす 0〜99 が
    ただ一つに絞られることを保証する。
    """
    pool = [
        ...
        ("素数である", _is_prime),
        ("3の倍数である", lambda n: n % 3 == 0),
        ("4の倍数である", lambda n: n % 4 == 0),
        ("7の倍数である", lambda n: n % 7 == 0),
        ("十の位 > 一の位 である", lambda n: (n // 10) > (n % 10)),
        ("十の位 < 一の位 である", lambda n: (n // 10) < (n % 10)),
        ("各桁の和が偶数である", lambda n: ((n // 10) + (n % 10)) % 2 == 0),
        ("各桁の和が10より大きい", lambda n: ((n // 10) + (n % 10)) > 10),
        ("各桁の和が8より小さい", lambda n: ((n // 10) + (n % 10)) < 8),
        ("40以上である", lambda n: n >= 40),
        ("60より小さい", lambda n: n < 60),
        ("25より大きい", lambda n: n > 25),
        ("75以下である", lambda n: n <= 75),
        ("十の位が偶数である", lambda n: (n // 10) % 2 == 0),
        ("一の位が奇数である", lambda n: (n % 10) % 2 == 1),
        ("ゾロ目ではない", lambda n: (n // 10) != (n % 10)),
        ("各桁の積が10より大きい", lambda n: (n // 10) * (n % 10) > 10),
        ("11以上である", lambda n: n >= 11),
    ]

    def find_hints(secret):
        """secret を満たす条件のうち、4〜5個で 0〜99 を secret 一つに絞れる組を探す。

        見つからなければ None。secret が満たす条件だけを母集団にするので、
        提示ヒントはすべて真であり、かつ解が一意になることが保証される。
        条件は20個程度と少ないので、組み合わせを網羅探索して確実に見つける。
        """
        satisfied = [(text, cond) for text, cond in pool if cond(secret)]
        for k in (4, 5):
            if len(satisfied) < k:
                continue
            combos = list(itertools.combinations(satisfied, k))
            random.shuffle(combos)  # 提示されるヒントの並び・組を毎回変える
            for combo in combos:
                candidates = [n for n in range(100) if all(cond(n) for _, cond in combo)]
                if candidates == [secret]:
                    return [text for text, _ in combo]
        return None

    # 一の位が answer に一致する 2桁(10〜99)の候補をシャッフルし、
    # 「論理ヒントだけで一意に解ける」secret が見つかるまで順に試す。
    # （0〜9 のどの answer でも、解ける secret が必ず存在することは検証済み。）
    secret_pool = [n for n in range(10, 100) if n % 10 == answer]
    random.shuffle(secret_pool)
    for secret in secret_pool:
        hints = find_hints(secret)
        if hints is not None:
            st.session_state.echo_logic_secret = secret
            st.session_state.echo_logic_hints = hints
            return

    # 念のためのフォールバック: 必ず一意になるよう桁を直接指定する
    secret = secret_pool[0]
    st.session_state.echo_logic_secret = secret
    st.session_state.echo_logic_hints = [
        f"十の位は {secret // 10} である",
        f"一の位は {secret % 10} である",
        "偶数である" if secret % 2 == 0 else "奇数である",
    ]
...
# ==========================================================================
# 全6色のパレット。ラウンドが進むほど使用色数が増える（序盤4色→後半6色）。
SIM_SYMBOLS = ["🟥", "🟦", "🟩", "🟨", "🟪", "🟧"]
SIM_START_LEN = 2   # 最初のラウンドの色数
SIM_GOAL = 6        # この長さまで再現できればクリア


def sim_palette_size(round_len):
    """ラウンド（提示する色列の長さ）に応じて使用する色数を返す。

    序盤は4色、長くなるほどパレットを広げて最大6色まで使う。
    """
    if round_len <= 3:
        return 4
    if round_len <= 4:
...
def sim_make_sequence(round_len):
    """指定長の色列を毎回新規生成する（継ぎ足しではなく毎回リフレッシュ）。"""
    palette = SIM_SYMBOLS[:sim_palette_size(round_len)]
    return [random.choice(palette) for _ in range(round_len)]


def setup_simon():
    """サイモンを初期ラウンド（最短の色列）で初期化する。"""
    st.session_state.rm_lab_len = SIM_START_LEN
    st.session_state.rm_lab_seq = sim_make_sequence(SIM_START_LEN)
...
def room_lab():
    ...
    st.write(
        ...
        "ラウンドごとに**まったく新しい色列**が提示され、長さも1つずつ伸びていく。"
        "進むほど**使用される色の種類**も増える。"
        f"提示された順番どおりに色を押し、**{SIM_GOAL}色**の列まで再現できれば認証が通る。"
    )
    ...
    # 現在ラウンドで使えるボタン（色数）はそのラウンドの長さに対応
    buttons = SIM_SYMBOLS[:sim_palette_size(len(seq))]

    if phase == "show":
        st.info(
            f"この **{len(seq)} 色** の順番を覚えよ。"
            f"（{len(seq)} / {SIM_GOAL} 色 ・ 使用色数 {len(buttons)}）"
        )
        st.markdown(
        ...
        for col, sym in zip(cols, buttons):
            if col.button(sym, key=f"sim_{sym}", use_container_width=True):
                ...
                if len(answer) == len(seq):
                    if len(seq) >= SIM_GOAL:
                        ...
                        # 次ラウンドへ：長さを1つ増やし、色列を“新規に”作り直す
                        new_len = len(seq) + 1
                        st.session_state.rm_lab_len = new_len
                        st.session_state.rm_lab_seq = sim_make_sequence(new_len)
                        st.session_state.rm_lab_phase = "show"
                        st.session_state.rm_lab_answer = []
        ...
        if st.session_state.rm_lab_msg == "fail":
            st.error("❌ 順番が違う。信号がリセットされ、新しい色列の最初からやり直しだ。")
...
def room_server():
    ...
    echo_say("server")
    st.write(
        "端末に**論理コード錠**が表示されている。錠の内部暗証番号は **00〜99 の2桁**。"
    ...
    st.info("💡 次の論理ヒントを**すべて同時に満たす 00〜99 の2桁は、ただ一つ**に絞られる。")
    st.markdown(
        "<div style='background:rgba(255,255,255,0.04); padding:12px 16px; border-radius:6px;'>"
        + "<br>".join(f"・暗証番号は {h}" for h in hints)
        + "</div>",
        unsafe_allow_html=True,
    )
    st.caption("入力するのは、確定した2桁の暗証番号の**一の位**（0〜9）。")

    ans = st.number_input("確定した暗証番号の一の位（0〜9）", min_value=0, max_value=9,
    ...
    if st.button("🔓 コードを入力", use_container_width=True):
        if ans is None:
            ...
            st.error("❌ 不一致。全てのヒントを同時に満たす2桁を一つに絞り、その一の位を入力せよ。")

    if st.session_state.rm_server_miss >= 2:
        st.caption(
            "（ヒント: 00〜99 を順に当て、各条件を全て満たす数を一つに絞れ。"
            "見つかった2桁の“右の桁”がコードだ）"
        )
...
def page_ending():
    ...
    if st.session_state.get("_in_portal"):
        if st.button("🏠 ポータルに戻る", use_container_width=True, key="echo_back_portal"):
